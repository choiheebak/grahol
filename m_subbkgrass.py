#
# 제목 : basketball 경기별 경기별 리그 상대전적 sub(NBA,KBL _일정결과) (subbkgras)
#
import pandas as pd
import sqlite3
...
def wdl_cnt(hteam, ateam, game):
    ...
    # ateamc = subbkwdlg.team_change(ateam)
    ...
    if league_gubun == 'N': #NBA
        sql = "SELECT  일자, 홈팀, 홈팀점수, 원정팀, 원정팀점수, 년도, 월 FROM NBA_일정결과 WHERE 홈팀 like ? AND 원정팀 like ? \
               UNION SELECT  일자, 홈팀, 홈팀점수, 원정팀, 원정팀점수, 년도, 월 FROM NBA_일정결과 WHERE 홈팀 like ? AND 원정팀 like ? order by 년도 , 월 , 일자 "  

    elif league_gubun == 'K': #KBL
        sql = "SELECT  일자, 홈팀, 홈팀점수, 원정팀, 원정팀점수, 년도, 월 FROM KBL_일정결과 WHERE 홈팀 like ? AND 원정팀 like ?   \
               UNION  SELECT  일자, 홈팀, 홈팀점수, 원정팀, 원정팀점수, 년도, 월 FROM KBL_일정결과 WHERE 홈팀 like ? AND 원정팀 like ? order by 년도 , 월 , 일자 "      
