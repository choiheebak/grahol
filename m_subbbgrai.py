#
# 제목 : baseball 경기별 상대전적 sub(MLB,KBO _일정결과) (subbbgrai)
#
import pandas as pd
import sqlite3
...
def wdl_cnt(hteam, game):
    ...
    if league_gubun == 'M': #MLB
        sql = "SELECT  일자, 홈팀, 홈팀점수, 원정팀, 원정팀점수 FROM MLB_일정결과 WHERE 홈팀 like ? OR 원정팀 like ? AND 홈팀점수 not in('') order by 년도 , 월 , 일자 "  

    elif league_gubun == 'K': #KBO
        sql = "SELECT  일자, 홈팀, 홈팀점수, 원정팀, 원정팀점수 FROM KBO_일정결과 WHERE 홈팀 like ? OR 원정팀 like ? order by 년도 , 월 , 일자 " 
